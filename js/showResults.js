...
for (let i = 0; i < timePicker.length; i++) {
  timePicker[i].addEventListener('change', (event) => {
    switch (event.target.id) {
      case 'starter':
        times.starter = event.target.value;
        break;
      case 'mainCourse':
        times.main = event.target.value;
        break;
      case 'dessert':
        times.dessert = event.target.value;
        break;
    }
...
for (let i = 0; i < textInputs.length; i++) {
  textInputs[i].addEventListener('change', (event) => {
    texts[event.target.id] = event.target.value;
    createMailLinks(groups);
  });
}
...
function createRow(group, i) {
  ...
  td.appendChild(document.createTextNode(group.name));
  tr.appendChild(td);
...
function generateMailContent(self, i) {
  ...
  for (i = 0; i < 6; i += 2) {
    ...
    if (otherGroups[i].meal === meal) {
      mealString = getMealString(meal, time, otherGroups[i]);
    }
